...
## 路由器组成
 ...
 ![image-20210605231614820](C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210605231614820.png)
...
Link state 链路状态：两个路由器之间链路的状态，包括关于路由器接口和它与相邻路由器之间关系的信息
...
* Router (config-if) # ip ospf priority number
 ...
 ![image-20210606173309303](C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210606173309303.png)

cost配置

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210606173354979.png" alt="image-20210606173354979" style="zoom:67%;" />

两种配置

**配置OSPFtimer**

T1线路的hello interval是10s   多路复用的hello interval是30s

默认：Dead interval = 4 * hello interval
...
### Symmetric Switching 对称交换

对称交换提供相同的带宽给端口间的交换连接 (10/10 Mbps或100/100 Mbps)

当用户试图访问别的网段上的服务器时，可能导致瓶颈

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210606174431136.png" alt="image-20210606174431136" style="zoom:67%;" />

### Asymmetric Switching 非对称交换

不对称交换通过用更高带宽(100 Mbps)的端口连接服务器的网段，来减少了服务器上潜在的瓶颈

不对称交换需要交换机内有存储缓冲区

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210606174445750.png" alt="image-20210606174445750" style="zoom:67%;" />

### Memory Buffering 内存缓冲区

交换机中的存储区域，存有目的地址和传输数据，直到数据被交换的正确的输出端口
...
- 交换机会增加延迟，使用切入交换方法可以减少延迟
- 快速转发交换—只检查目标MAC，接着立刻转发帧 （转发速度很快，但不管是否有错误）
- fragment free自由分段—读取开头的64字节来减少转发的错误

### 交换机

第二层交换机和第三层交换机

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210606180043074.png" alt="image-20210606180043074" style="zoom:50%;" />![image-20210606180109631](C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210606180109631.png)<img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210606180043074.png" alt="image-20210606180043074" style="zoom:50%;" />![image-20210606180109631](C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210606180109631.png)

四层交换机和多层交换机

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210606180654122.png" alt="image-20210606180654122" style="zoom:50%;" /><img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210606180708270.png" alt="image-20210606180708270" style="zoom:50%;" />

## The Spanning-Tree Protocols 生成树协议
...
- 通常因为蓄意提供冗余，导致网络中的循环
- 也可能是配置错误
  - 以下两个原因出现在网络中的循环是灾难性的：
    - 广播循环
    - 过时的网桥表

### 冗余构造循环

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607080901375.png" alt="image-20210607080901375" style="zoom: 50%;" />
...
- 以太帧没有TTL域
- 在一个以太帧开始循环后，它会一直进行直到有人关掉了其中一个交换机或打断一个链接
- 交换机会触发主机A的网桥表条目 (导致CPU很高的使用)

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607081130029.png" alt="image-20210607081130029" style="zoom: 50%;" />
...
- STP通过计算一个稳定的生成树网络拓扑来阻止循环 
- 生成树帧 (又叫桥接协议数据单元BPDUs) 用于决定生成树拓扑

STP 决策序列

生成树通常使用同样的4步决策序列：

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607081729884.png" alt="image-20210607081729884" style="zoom:50%;" />

- 最低根的BID (桥识别符Bridge Identification) 
- 根网桥的最低路径开销
- 最低发送BID
- 最低端口ID 

一开始每个switch把自己作为root，cost为0.开始交换信息时，将BID更小的作为根，等网络稳定了以后将公共root的端口作为唯一对外的端口，以此来避免回路
...
- BID高位子域(2字节)：网桥优先级
  - 216种可能的值：0-65,535 (默认：32,768)
  - 通常表示为十进制形式
- 低位子域(6字节)：交换机被分配的MAC地址 
  - 表示为十六进制形式
...
- 通过寻找有着最低BID的交换机(通常被认为是网桥战争)，来选择根交换机.
- 如果所有交换机使用默认网桥优先级32,768，那么**MAC地址最低**的就是打破平局的关键
...
- 默认的STP状态命令有：
  - 阻塞Blocking—不作为端口，没有帧转发，接收BPDUs
  - 侦听Listening--没有帧转发，侦听数据帧
  - 学习Learning--没有帧转发，学习地址
  - 转发Forwarding--有帧转发，学习地址
  - 禁用Disabled--没有帧转发，不接受BPDUs

Initial STP Convergence 最初的STP集合

- 当网络刚开始，所有网桥向网络中发送BPDU信息
- 他们立刻在BPDU集合上应用决策队列，来得出整个网络的生成树

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607083249572.png" alt="image-20210607083249572" style="zoom:67%;" />

步骤一 选择根网桥：选择一个根网桥来作为网络的中心

- 宣布它自己是根
- 检查端口收到的所有BPDUs和即将在那个端口发送的BPDU
- 对于每一个收到的BPDU，如果比该端口保存的BPDU的值低
  - 旧的值被取代
  - BPDU的发送方成为新的根

步骤二 选择根端口：所有剩余网桥计算根端口的集合

- 每一个不是根的网桥必须选择一个根端口
  - 网桥的根端口是离根网桥最近的端口
  - 根路径开销是到达根网桥的所有链接开销的累计

步骤三 选择指定端口：所有剩余网桥计算指定端口的集合
...
- 选择指定端口(II)
  ...
  <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607083557211.png" alt="image-20210607083557211" style="zoom:50%;" />
...
LANs & VLANs的不同之处
 ...
 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607084356226.png" alt="image-20210607084356226" style="zoom:67%;" />
...
- 用户可以通过软件逻辑分组，基于：端口号/MAC地址/使用的协议/使用的应用
...
frame filtering帧过滤—检查关于每个帧中特定的信息 (MAC地址或第3层协议类型) 

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607085537964.png" alt="image-20210607085537964" style="zoom: 50%;" />

frame tagging帧标记—当帧通过网络骨干被转发的时候，在它开头放一个独特的VLAN标识符

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607085626609.png" alt="image-20210607085626609" style="zoom:50%;" />
...
静态VLANs是当交换机上的端口被管理员分配VLAN

好处：安全，容易配置和监控/因为控制移动，所以在网络中表现好

**动态VLANs**

当一个站点最初连接在一个没有分配的端口上，交换机检查表中的条目，然后动态地配置正确的VLAN到端口上

好处：当增加或移除用户的时候，管理更少（更提前）/集中没有被授权的用户的通知

**以端口为中心的VLANs**

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607090236087.png" alt="image-20210607090236087" style="zoom:50%;" />
...
#### Access Links and Trunk Links 访问链路和主干链路
...
访问和主干链路也存在
 ...
 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607090736515.png" alt="image-20210607090736515" style="zoom:67%;" />
...
![image-20210607092249123](C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607092249123.png)

### WAN Virtual Circuits

Switched Virtual Circuits 虚电路交换 SVCs

交换虚拟回路 (SVCs)是依据需求建立和终止的，到达目的地址的广域网路径

**三个阶段**

Circuit establishment 回路建立 – 建立虚拟回路
Data Transfer 数据传输 – 发送和接收用户数据
Circuit termination 回路终止 – 拆掉虚拟回路

使用：电话服务和ATM使用SVCs

 ![image-20210607092958825](C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607092958825.png)
...
* 访问服务提供者的回路
* 有一个合适可用的路由端口
* 一个CSU/DSU，调制解调器，ISDN 终端适配器等

### Modem 调制解调器

也叫CSU/DSUs信道服务单元/数据服务单元 (channel service units/digital service units) 
有着话音级连接的接口来将模拟信号转成数字信号

![image-20210607095022455](C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607095022455.png)
...
广域网数据链路层定义了为了传输到遥远的网站，数据是怎样封装的

* 点对点通讯协定(PPP)：由IETF发展。PPP包括了一个协议域来识别网络层协议
* 高级数据链路控制(HDLC)：一个ISO标准。HDLC不能兼容不同的供应商，因为每个供应商实现的方式不同。HDLC支持点对点/多点配置。
* 帧中继：在高质量的数码设备上使用没有错误校正特定封装
* 综合业务数字网ISDN：一个数字服务的集合，通过现有电话线传输声音和数据
* 平衡型链路接入规程(LAPB)：给包交换的网络，用于在X.25栈的第2层封装包。基于点对点提供可靠性和流量控制
...
两个最常见的点对点广域网封装是HDLC和PPP

所有串行线封装共享一个常见的帧格式，有着下列域

对于封装协议的选择，由广域网技术和通讯设备决定

**PPP是一个标准的串行封装方式**
由IETF发展，取代了SLIP
包括一个用于识别网络层协议的域
PPP可以在连接建立时，检查链路质量
通过通行码鉴别协定 (PAP)和挑战握手鉴别协定 (CHAP)提供鉴别

第2层协议中在串行链路上使用最广泛的协议
从SLIP发展而来，SLIP
仅支持IP协议
不支持动态IP分配
不支持鉴别
...
使用HDLC(ISO HDLC, 不是思科HDLC) 作为封装第3层数据报的基础
LCP (Link Control Protocol)用于：

* 连接建立
* 链接配置选项
* 链接质量测试

NCP (Network Control Protocol)用于选择和配置第3层协议

**PPP帧格式**

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607100135726.png" alt="image-20210607100135726" style="zoom:50%;" />

比较固定，地址为全1广播，控制默认为3，数据最大1500字节，FCS由传输双方确定是2/4字节

#### PPP会话建立/终止

* 链接建立和配置协商(LCP)

   **在任何网络层数据报可以交换之前，链接建立是第一阶段**

  * 每个PPP设备发送LCPs，来打开连接 
  * LCP包包括一个允许设备协商选项使用的配置选项域 ，如压缩和鉴别协议等
    如果一个配置选项不在LCP包中，假定这个选项为默认值
  * 当配置应答帧已经被发出和接收，这个阶段就完成了

* 链接质量测试 
  * 如果被配置，发送和接收LCP包来测量链接的错误率 
  * 如果使用，鉴别发生在网络层协议配置开始之前
  * LCP可以延迟网络层协议信息的发送直到这个阶段完成

* 网络层协议配置(NCP)
  * 在这个阶段，PPP设备发送NCP包来选择和配置一个或更多的网络层协议(如IP). 
  * 当每个被选中的网络层协议已经配置完成，来自每个网络层协议的数据报就可以在链接上传送
* 链接终止
  * LCP可以在任何时间终止链接：
    * 在用户请求的时候
    * 链接质量
    * 超时
    * 当LCP关闭链接，它会通知网络层协议，使得它们可以做出合适的行为

**HDLC是思科默认的串行封装方式**
没有窗口或流量控制
帧中插入了一个专利类型代码，代表HDLC帧并不兼容其它供应商的设备 
***只有专线两端的连接都是运行思科IOS的路由器，HDLC才能被使用***
...
* 篡改——故意篡改网络上传送的报文。
* 伪造——伪造信息在网络上传送。
* <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607110422619.png" alt="image-20210607110422619" style="zoom:50%;" /> 
...
<img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607111005769.png" alt="image-20210607111005769" style="zoom:67%;" />
...
<img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607111222007.png" alt="image-20210607111222007" style="zoom:67%;" />
...
### 公钥密码体制
 ...
 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607111452630.png" alt="image-20210607111452630" style="zoom: 67%;" />

![image-20210607111519071](C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607111519071.png)
...
<img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607112331799.png" alt="image-20210607112331799" style="zoom:67%;" />

因为除 A 外没有别人能具有 A 的私钥，所以除 A 外没有别人能产生这个密文。因此 B 相信报文 X 是 A 签名发送的。
若 A 要抵赖曾发送报文给 B，B 可将明文和对应的密文出示给第三者。第三者很容易用 A 的公钥去证实 A 确实发送 X 给 B。
反之，若 B 将 X 伪造成 X‘，则 B 不能在第三者前出示对应的密文。这样就证明了 B 伪造了报文。

![image-20210607112430393](C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607112430393.png)

## 防火墙

对终端的加密

 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607112818066.png" alt="image-20210607112818066" style="zoom: 50%;" />

防火墙是由软件、硬件构成的系统，是一种特殊编程的路由器，用来在**两个网络之间**实施接入控制策略。接入控制策略是由使用防火墙的单位自行制订的，为的是可以最适合本单位的需要。
...
**例**
 ...
 <img src="C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607113146713.png" alt="image-20210607113146713" style="zoom:67%;" />

路由器如何使用ACL？

* 检查报文是否可路由，如果是，查路由表
* 检查ACL
  * 如果没有ACL switch the packet out the destination interface
  * 如果有ACL check the packet against the ACL statements sequentially

![image-20210607113738319](C:\Users\Edward\AppData\Roaming\Typora\typora-user-images\image-20210607113738319.png)